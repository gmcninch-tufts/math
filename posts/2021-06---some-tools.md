...
be able to do with ``gap`` (something of a trailer for the tutorial,
maybe?)...
...
It will print any output (or errors) from the command on the screen, and then it
will display another prompt and again wait for input.
...
an $n$-cycle and a transposition. In ``Gap``, We can check this for
...
over the field of order $q$  ``G:=GL(n,q);``; for example:
...
1 & x \\
0 & 1
\end{bmatrix} \mid x \in \mathbb{F}_{13}.\right\}$$
